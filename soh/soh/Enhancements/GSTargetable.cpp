...
static void OnActorInitGSTargetable(void* refActor) {
    Actor* enSw = reinterpret_cast<Actor*>(refActor);

    if (enSw->naviEnemyId == 0x20) {
        // Enable Targeting this Gold Skulltula, if visible by default
        if (enSw->scale.x >= 0.0139999995f) {
            enSw->flags |= ACTOR_FLAG_ATTENTION_ENABLED;
        }
        ...
        Actor_ChangeCategory(gPlayState, &gPlayState->actorCtx, enSw, ACTORCAT_MISC);
    }
}
...
static void OnActorUpdateGSTargetable(void* refActor) {
    ...
    if ((enSw->naviEnemyId == 0x20) && (((enSw->params & 0xE000) >> 0xD) == 2) &&
        (enSw->colChkInfo.health > 0)) {
        if (enSw->scale.x < 0.0139999995f) {
            // Night GS Despawn
            enSw->flags &= ~ACTOR_FLAG_ATTENTION_ENABLED;
        }
