...
static void UseTunicBoots(Player* player, PlayState* play, Input* input) {
    ...
    if (item >= ITEM_SHIELD_DEKU && item <= ITEM_BOOTS_HOVER) {
        if (item >= ITEM_BOOTS_KOKIRI) {
            ...
            const u16 shieldValue = item - ITEM_SHIELD_DEKU + 1;
...
static void ClearDeletedAssignedEquipment(int16_t equipmentType, uint16_t equipValue) {
    ...
    if (equipmentType == EQUIP_TYPE_TUNIC) {
        if (equipValue == EQUIP_VALUE_TUNIC_GORON) itemToRemove = ITEM_TUNIC_GORON;
        if (equipValue == EQUIP_VALUE_TUNIC_ZORA) itemToRemove = ITEM_TUNIC_ZORA;
    } else if (equipmentType == EQUIP_TYPE_SHIELD) {
        if (equipValue == EQUIP_VALUE_SHIELD_DEKU) itemToRemove = ITEM_SHIELD_DEKU;
        if (equipValue == EQUIP_VALUE_SHIELD_HYLIAN) itemToRemove = ITEM_SHIELD_HYLIAN;
        if (equipValue == EQUIP_VALUE_SHIELD_MIRROR) itemToRemove = ITEM_SHIELD_MIRROR;
    }
    if (itemToRemove == ITEM_NONE) return;
