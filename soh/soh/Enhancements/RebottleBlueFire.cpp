...
void OnRebottleBlueFireUpdate(void* actor) {
    EnIceHono* flame = (EnIceHono*)actor;
    if (flame->actionFunc != EnIceHono_CapturableFlame && EnIceHono_InBottleRange(flame, gPlayState)) {
        Actor_OfferGetItem(&flame->actor, gPlayState, GI_MAX, 60.0f, 100.0f);
    }
}
