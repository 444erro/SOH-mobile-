...
static void RegisterFasterShadowShip() {
    COND_VB_SHOULD(VB_SHADOW_SHIP_SET_SPEED, CVarGetInteger(CVAR_ENHANCEMENT("FasterShadowShip"), 0), {
        ...
        if (!Play_InCsMode(play)) {
            if (Actor_FindNearby(play, &ship->dyna.actor, ACTOR_EN_TEST, ACTORCAT_ENEMY, 800.0f) == NULL &&
                Actor_FindNearby(play, &ship->dyna.actor, ACTOR_PLAYER, ACTORCAT_PLAYER, 450.0f) != NULL) {
                *should = false; ship->dyna.actor.speedXZ = 25.0f;
            } else if (ship->dyna.actor.speedXZ > 4.0f) {
                *should = false; ship->dyna.actor.speedXZ = 4.0f;
            }
        }
    });
}
