...
static void ExporterProgramEnd()
{
	...
	if (Globals::Instance->fileMode == ZFileMode::ExtractDirectory)
	{
		...
		otrArchive->AddFile("version", (uintptr_t)versionStreamBuffer.data(), versionStream->GetLength());

		printf("Adding portVersion file.\n");
		auto portVersionStreamBuffer = portVersionStream->ToVector();
		otrArchive->AddFile("portVersion", (uintptr_t)portVersionStreamBuffer.data(), portVersionStream->GetLength());
	...
	printf("Adding portVersion file.\n");
	auto portVersionStreamBuffer = portVersionStream->ToVector();
	customOtr->AddFile("portVersion", (uintptr_t)portVersionStreamBuffer.data(), portVersionStream->GetLength());
