...
void ResourceManager::Init(const std::vector<std::string>& archivePaths,
    ...
    // The extra `- 1` reserves one thread for spdlog. Keep the arithmetic signed:
    // hardware_concurrency() may return zero or fewer cores than the reservations.
    const int32_t logicalCores = static_cast<int32_t>(std::thread::hardware_concurrency());
    size_t threadCount = static_cast<size_t>(std::max(1, logicalCores - reservedThreadCount - 1));
#if defined(__ANDROID__)
    // Large resource pools create memory/CPU spikes on phones without improving frame time.
    // Scale conservatively so audio and the game thread retain enough scheduling time.
    size_t androidThreadLimit = 2;
    const long pages = sysconf(_SC_PHYS_PAGES);
    const long pageSize = sysconf(_SC_PAGE_SIZE);
    if (pages > 0 && pageSize > 0) {
        const double memoryGiB = (static_cast<double>(pages) * static_cast<double>(pageSize)) /
                                 (1024.0 * 1024.0 * 1024.0);
        androidThreadLimit = memoryGiB <= 3.0 ? 1 : (memoryGiB <= 6.0 ? 2 : 4);
    }
    threadCount = std::clamp(threadCount, size_t{ 1 }, androidThreadLimit);
#endif
