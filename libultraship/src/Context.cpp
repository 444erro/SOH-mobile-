...
bool Context::InitResourceManager(const std::vector<std::string>& archivePaths,
    ...
    // Shared-storage paths saved by older Android builds are no longer valid
    // under scoped storage. Always use the protected canonical mods folder.
    mMainPath = GetAppDirectoryPath();
    mPatchesPath = GetAppDirectoryPath() + "/mods";
    std::filesystem::create_directories(mPatchesPath);
#endif
...
#if defined(__ANDROID__)
static std::string GetAndroidSohDataDirectory() {
    ...
    jmethodID getPath = env->GetMethodID(activityClass, "getSohDataDirectoryPath", "()Ljava/lang/String;");
    std::string result;
    if (getPath != nullptr) {
        jstring javaPath = static_cast<jstring>(env->CallObjectMethod(activity, getPath));
        if (javaPath != nullptr) {
...
#if defined(__ANDROID__)
    const std::string dataDirectory = GetAndroidSohDataDirectory();
