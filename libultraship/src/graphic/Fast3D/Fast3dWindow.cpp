...
void Fast3dWindow::SetMsaaLevel(uint32_t value) {
    // Multisample counts are discrete. Passing values such as 3/5/6/7 creates
    // incomplete framebuffers on several Android GLES drivers.
    gfx_msaa_level = value >= 8 ? 8 : value >= 4 ? 4 : value >= 2 ? 2 : 1;
}
